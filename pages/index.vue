...
import { rgb, colorsFromRange } from '@thi.ng/color';
...
onLoop(({ delta, elapsed }) => {
  if (blobRef.value) {
    ...
    if (mobile) {
      blobRef.value.material.uniforms.uPosition.value = new Vector3(
...
void main() {
  ...
  vUV = uv * vec2(1.0, uResolution.y / uResolution.x) - vec2(0.0, (uResolution.y / uResolution.x - 1.0) / 2.0);
  if (uIsSmall) {
    vUV *= 0.4;
    vUV += vec2(0.3, 0.3);
  }
  vMpos = position;
  float zdisp = fbm(position + normal * uJitter + uTime * 0.1) * 0.1;
  gl_Position = projectionMatrix * modelViewMatrix * vec4(position + normal * zdisp, 1.0);
}
`;
const fragshader = `
uniform float uTime;
...
float sdSphereNoise( vec3 p, float s )
{
  return length(p)-(fbm(uPosition+p)*s);
}

vec3 rgbSphere(vec3 p, float t, vec3 mousePos, float r) 
{
  vec3 c1 = vec3(step(sdSphereNoise(p-mousePos*sin(t)*0.05, 0.2),r),step(sdSphereNoise(p-mousePos*cos(t)*0.05, 0.1),r),step(sdSphereNoise(p-mousePos*0.1, 0.1),r));
  return c1;
}

void main() {
  // parallax effect using screen position
  vec3 pos = vec3(vUV.x - 0.5, vUV.y - 0.5, 0.0);
  vec3 color = rgbSphere(pos, uTime, uPosition, 0.175);
  float a = 1.0;
